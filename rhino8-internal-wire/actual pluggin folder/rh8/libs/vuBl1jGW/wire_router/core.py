...
def route_node_sequence(
    ...
    # that paths NOT connecting to that node should avoid.
    #
    # `reserved_cells` already blocks the full physical node volume.
    # Applying the entire inter-wire blocked radius on top of that to
    # every future node is often too restrictive for the first segment,
    # causing early no-route failures in dense layouts.  Use a moderate
    # additional margin here and keep the stronger inter-segment spacing
    # rules for routed wires themselves.
    node_keepout_radius = reserved_exemption_radius + (blocked_radius // 2) if blocked_radius > 0 else 0
    ...
    def _search(
        ...
        if per_node_keepout and segment_index > 0:
