/*
 * 4-Node Bidirectional Touch Sensor (High-R Test Variant)
 * --------------------------------------------------------
 * Dedicated sketch for new high-resistance prints.
 *
 * Learned constraints from previous runs:
 *   - Keep bidirectional fwd/rev sensing (2D signature)
 *   - Keep long per-node calibration (7s, pressure sweep)
 *   - Keep switch hysteresis and ambiguity rejection
 *   - Add stronger touch-entry stability gating for onset transients
 *
 * WIRING:
 *
...
// ── Pin config ───────────────────────────────────────────────────────
#define SEND_PIN    5   // through external resistor to trace start
...
#define ENTRY_TOUCH_GATE_COUNT 3
#define ENTRY_SUM_MARGIN      25
#define ENTRY_DIFF_MARGIN     20
#define ENTRY_SCORE_GAP_MIN   0.80f
#define INVALID_FRAME_LIMIT   900000UL
...
int logicalIdToIdx(int logicalId) {
  return logicalId - 1;
}

int physicalIndexForLogicalId(int logicalId) {
  for (int p = 0; p < NUM_NODES; p++) {
    if (physicalNodeOrder[p] == logicalId) return p;
  }
  return -1;
}
...
void printPhysicalOrder() {
  Serial.print(F("  Physical node order (START→END): "));
  for (int i = 0; i < NUM_NODES; i++) {
    if (i > 0) Serial.print(F(" -> "));
    Serial.print(F("N"));
    Serial.print(physicalNodeOrder[i]);
  }
  Serial.println();
}

bool isPhysicalAdjacent(int logicalNodeAIdx, int logicalNodeBIdx, int &segmentIdxOut) {
  int logicalA = logicalNodeAIdx + 1;
  int logicalB = logicalNodeBIdx + 1;
  int physA = physicalIndexForLogicalId(logicalA);
  int physB = physicalIndexForLogicalId(logicalB);
  if (physA < 0 || physB < 0) return false;

  if (abs(physA - physB) == 1) {
    segmentIdxOut = min(physA, physB);
    return true;
  }
  return false;
}
...
void setup() {
  ...
  initNodeResistanceMap();

  Serial.println();
  Serial.println(F("====================================================="));
  Serial.println(F("  4-Node Bidirectional Touch Sensor (High-R Test)"));
  Serial.println(F("====================================================="));
  Serial.println();
  ...
  Serial.println(F("  Touch logical nodes in order N1, N2, N3, N4 when prompted."));
  Serial.println(F("  Path order may differ; physical-order map is printed above."));
  ...
  for (int i = 0; i < NUM_NODES; i++) {
    ...
    Serial.print(i + 1);
    Serial.print(F("  ("));
    Serial.print(nodeResKohm[i], 1);
    Serial.print(F("k from START, physical #"));
    Serial.print(physicalIndexForLogicalId(i + 1) + 1);
    Serial.println(F(") <<<"));
  ...
  Serial.println();
  Serial.println(F("  Node | R(k) from START | fwdB       | revA       "));
  Serial.println(F("  ─────|─────────────────|────────────|────────────"));
  ...
  Serial.println();
  Serial.println(F("  2D distances between logical nodes:"));
  bool allGood = true;
  ...
  Serial.println();
  Serial.println(F("  Adjacent pairs by physical trace order:"));
  for (int p = 0; p < NUM_NODES - 1; p++) {
    int nodeA = physicalNodeOrder[p] - 1;
    int nodeB = physicalNodeOrder[p + 1] - 1;

    long d = (long)sqrt((double)dist2(nodeFwdMean[nodeA], nodeRevMean[nodeA],
                                       nodeFwdMean[nodeB], nodeRevMean[nodeB]));
    long maxStd = max(max(nodeFwdStd[nodeA], nodeRevStd[nodeA]),
                      max(nodeFwdStd[nodeB], nodeRevStd[nodeB]));
    float sigma = 9999.0f;
    if (maxStd > 0) sigma = (float)d / (float)maxStd;
  ...
  float totalPathNomK = (totalPathRangeKohmMin + totalPathRangeKohmMax) * 0.5f;
  float extRatio = totalPathNomK > 0.0f ? (EXT_RES_KOHM / totalPathNomK) : 0.0f;
  ...
  Serial.print(F("  Total path range: ")); Serial.print(totalPathRangeKohmMin, 1);
  Serial.print(F("k to ")); Serial.print(totalPathRangeKohmMax, 1); Serial.println(F("k"));
  Serial.print(F("  Total path nominal: ~")); Serial.print(totalPathNomK, 1); Serial.println(F("k"));
  ...
  if (!allGood && minAdjSegIdx >= 0) {
    ...
    Serial.print(F("     Increase segment between N")); Serial.print(nodeA);
    Serial.print(F(" and N")); Serial.print(nodeB); Serial.println(F("."));
    ...
    Serial.print(F("     That means cumulative N")); Serial.print(nodeB);
    Serial.print(F(" from ")); Serial.print(nodeResKohm[logicalIdToIdx(nodeB)], 1);
  ...
  if ((minSigmaAdj < SIGMA_WARN_TH) && (minAdjSegIdx >= 0)) {
    Serial.print(F("  Adjacent bottleneck: N")); Serial.print(physicalNodeOrder[minAdjSegIdx]);
    Serial.print(F(" ↔ N")); Serial.print(physicalNodeOrder[minAdjSegIdx + 1]);
...
void loop() {
  ...
  if (fwdRaw >= INVALID_FRAME_LIMIT || revRaw >= INVALID_FRAME_LIMIT) {
    if (millis() - lastPrintMs >= 200) {
      lastPrintMs = millis();
      Serial.print(F("[invalid frame] fwd=")); Serial.print((long)fwdRaw);
      Serial.print(F(" rev=")); Serial.println((long)revRaw);
    }
    return;
  }
