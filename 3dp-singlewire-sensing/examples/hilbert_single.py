...
if __name__ == "__main__":
    mesh_file_name = "hilbert"
    surface = pv.get_reader(f"../models/{mesh_file_name}.stl").read()
    surface = surface.scale(1.4)
    # surface.save(f"../models/{mesh_file_name}_scaled_scaled.stl")
    with open(f"../models/selection_{mesh_file_name}.json", "r") as f:
        set_of_seleceted_vertex_indices = json.load(f)
    # remove the last connection node to make a single connection
    set_of_seleceted_vertex_indices = set_of_seleceted_vertex_indices[:-1]
    ...
    voxel_graph = gu.pointset_to_graph(inner_voxels, n_neighbors=6)
    ...
    ## for previewing links and surface
    ...
    # # NOTE: run with "aiming_resistance": 2000e3 above then will see
    # # optimized resitances: [795872.9906981249, 541801.6211031071, 1140304.9702118689, 1791414.9884893675]
    # # so then use below code to know optimize resistance: wire: 1.9 M, trace_r: 150.0 k => i.e., 500k in code
    # (0.23 is adjustment of esimated to real-world resistance)
    # n_nodes = len(nodes)
    # voltage_thres = 2.5
    ...
    #     trace_resistance_candidates=[450e3 * 0.23],
    # )
    # print(
    #     f"wire: {wire_r}, trace_r: {trace_r}"
    # )
    with open(f"../models/{mesh_file_name}_wire_resistance.txt", "w") as f:
        f.write("1.5 M\n")
