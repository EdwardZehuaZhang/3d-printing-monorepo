...
def trace_to_cylinders(
    ...
    # Use a simple combination function instead of sensing_network import
    return _combine_polydata_objects(cylinders)


def _combine_polydata_objects(objects):
    """Simple helper to combine polydata objects"""
    if len(objects) == 0:
        return pv.PolyData()
    combined = objects[0]
    for obj in objects[1:]:
        combined = combined.merge(obj)
    return combined
