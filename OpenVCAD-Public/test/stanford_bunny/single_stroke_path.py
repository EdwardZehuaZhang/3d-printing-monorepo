"""
Legacy Single-Stroke Path Generator (OBJ-only)
Reimplements the original method: parse OBJ vertex lines, sample, k-NN → MST → DFS
and render a GraphLattice trace with the classic parameters.
"""
import pyvcad as pv
...
def load_mesh_vertices_obj(obj_path: Path) -> np.ndarray:
    verts = []
...
def generate_curve_points(obj_path: Path, num_points=500, k_neighbors=5):
    print(f"Loading mesh (OBJ) from: {obj_path}")
    verts = load_mesh_vertices_obj(obj_path)
...
def main():
    ...
    curve_pts = generate_curve_points(obj_file, num_points=500, k_neighbors=5)
    struts = [(curve_pts[i], curve_pts[i + 1]) for i in range(len(curve_pts) - 1)]
    print(f"Creating GraphLattice with {len(struts)} struts...")
    trace = pv.GraphLattice(struts, 0.0008, materials.id("white"))

    print("Rendering legacy trace. Uncomment viz.Export to write STL.")
